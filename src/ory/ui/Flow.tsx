...
import type { FlowProps, FlowState, UpdateValues } from '@/types';

function emptyState<T>() {
    return {} as T;
}

export class Flow<T extends UpdateValues> extends Component<FlowProps<T>, FlowState<T>> {
    ...
    initializeValues = (nodes: Array<UiNode> = []) => {
        ...
        nodes.forEach((node) => {
            ...
            if (isUiNodeInputAttributes(node.attributes)) {
                ...
                values[node.attributes.name as keyof UpdateValues] = node.attributes.value;
            }
        });
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                {nodes.map((node, k) => {
                    const id = getNodeId(node) as keyof UpdateValues;
                    return (
                        <Node
                            key={`${id}-${k}`}
                            disabled={isLoading}
